# ---
# description: Numerical evaluation of the Euler-Maclaurin remainder bound for the rounding second-moment constant
# ---
import numpy as np
...
def compute_bound(k: float, n: int) -> float:
    """Euler-Maclaurin remainder bound (times Lambda^2) at even derivative order n for the rounding
    second moment, where g(t) = (t/Lambda)^2 [e^{-t k^{-1/2}} - e^{-t k^{1/2}}]. Since
    f(x) = g(Lambda k^x) satisfies f^{(n)}(x) = (ln k)^n L^n g(t) with L = t d/dt, the n-th derivative
    is integrated exactly as a single (polynomial)x(exponential), keeping the cancellation between the
    two exponential terms."""
    a, b = k**-0.5, k**0.5
    Qa, Qb = compute_Q(n, a, 2), compute_Q(n, b, 2)

    def integrand(t: float) -> float:
        return t * abs(P.polyval(t, Qa) * np.exp(-a * t) - P.polyval(t, Qb) * np.exp(-b * t))
