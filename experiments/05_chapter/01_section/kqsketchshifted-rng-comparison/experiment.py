# ---
# description: Mean addition time and mean relative error vs m for kQSketchShifted across 4 RNG engines, combined into one figure with 2 subplots
# ---
"""
Measures mean addition time (μs/element) and mean relative error as a function
of sketch size m for kQSketchShifted, separately for each of the four RNG engines.
...
# BENCHMARK: DRAFT=0: >80s, DRAFT=1: ~57s, DRAFT=2: 1.1s.
match DRAFT:
...
def main() -> None:
    ...
    pbar = tqdm.tqdm(total=TOTAL_PBAR_VALUE, desc="kQSketchShifted RNG comparison", colour="green", unit="pt", mininterval=10.0)

    rng: Generator = np.random.default_rng(100)
    ...
    for rng_name, rng_enum in RNGS:
        ...
        for m in ms:
            ...
            for rep in range(N_REPS):
                sk = kQSketchShifted(m, int(rng.integers(0, 2**63)), amount_bits=8, logarithm_base=2.0, rng_engine=rng_enum)
